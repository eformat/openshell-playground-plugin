...
func (s *server) handleNamespaces(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, 500, fmt.Sprintf("failed to list namespaces: %v", err))
		return
	}
...
func (s *server) handleProviders(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		output, err := execInGateway(r.Context(), s.client, ns, "openshell provider list --output json")
		if err != nil {
	...
	case http.MethodPost:
		...
		_, err := execInGateway(r.Context(), s.client, req.Namespace, args)
		if err != nil {
...
func (s *server) handleAgents(w http.ResponseWriter, r *http.Request) {
	...
	inferProvider := ""
	if out, err := execInGateway(ctx, s.client, ns, "openshell inference get --output json"); err == nil {
		var inferData map[string]interface{}
		if json.Unmarshal([]byte(out), &inferData) == nil {
			if p, ok := inferData["provider"].(string); ok {
				inferProvider = p
			}
		}
	}

	list, err := s.dynClient.Resource(sandboxGVR).Namespace(ns).List(ctx, metav1.ListOptions{})
	if err != nil {
...
func (s *server) handleDeleteAgent(w http.ResponseWriter, r *http.Request, name, ns string) {
	...
	_, err := execInGateway(r.Context(), s.client, ns, cmd)
	if err != nil {
...
func (s *server) handleDeploy(w http.ResponseWriter, r *http.Request) {
	...
	// Enable providers v2
	_, err := execInGateway(ctx, s.client, req.Namespace,
	...
	if req.Provider != "" {
		...
		_, err = execInGateway(ctx, s.client, req.Namespace, inferCmd)
		if err != nil {
	...
	for i := 1; i <= req.Count; i++ {
		...
		_, err := execInGateway(ctx, s.client, req.Namespace, createCmd)
		if err != nil {
...
func (s *server) handleGatewayPod(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || len(pods.Items) == 0 {
		pods2, err2 := s.client.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{})
		if err2 != nil {
